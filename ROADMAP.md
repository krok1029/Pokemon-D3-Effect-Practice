...
更新日期：2026-09-26。#14～#25 已實作並有驗收紀錄；以下區分已交付功能、現有限制與尚未實作的候選。母規格見 [#13](https://github.com/krok1029/Pokemon-D3-Effect-Practice/issues/13)。
...
驗收文件保存各批次當時的執行環境、通過／失敗及修正結果；其中舊的「剩餘限制」是歷史狀態。目前進度以本文件及實作為準。
...
- **散佈圖互動**：不持久化跨頁選取／縮放。手機與鍵盤可使用搜尋選取入口，仍未提供手機拖曳框選。
...
| Ticket                                                                  | 補強結果                                                 | 驗收紀錄                                                |
| ----------------------------------------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------- |
| [#21](https://github.com/krok1029/Pokemon-D3-Effect-Practice/issues/21) | 深層圖鑑點主要導覽後回第一批；捲動更新網址仍保留累積列表 | [導覽修正](docs/verification/catalog-navigation-fix.md) |
| [#22](https://github.com/krok1029/Pokemon-D3-Effect-Practice/issues/22) | 同頁切換傳說條件保留能力軸與主屬性偏好，移除不符合的選取 | [條件保留](docs/verification/chart-filter-state-fix.md) |
| [#23](https://github.com/krok1029/Pokemon-D3-Effect-Practice/issues/23) | 放大／平移後的資料點裁切於繪圖區，保留座標軸及框選       | [畫面裁切](docs/verification/chart-clipping-fix.md)     |
| [#24](https://github.com/krok1029/Pokemon-D3-Effect-Practice/issues/24) | 鍵盤／觸控可搜尋、選取並開啟指定型態，下拉選單有明確標籤 | [操作驗收](docs/verification/chart-accessibility.md)    |
| [#25](https://github.com/krok1029/Pokemon-D3-Effect-Practice/issues/25) | 隔離正式版 E2E、CI 與平均圖表數值／操作回歸              | [回歸環境](docs/verification/regression-ci.md)          |

前三項修正已重現的功能問題，後兩項補足操作與交付驗證。

## 後續候選：定義 2～4 隻寶可夢比較

前置的第二次盤點修正已涵蓋散佈圖選取後平移、暗色圖表文字、搜尋格式、載入逾時及 CSV 整數驗證；各項結果與回歸入口見 [平行修正驗收](docs/verification/parallel-fixes-2026-09-26.md)。
...
另有較小的既有介面缺口待整理：詳細頁缺少 `h1`、無效型態沿用預設英文 404，以及 Geist 字型 class 未正確套用。本輪五項修正未包含這些項目。
