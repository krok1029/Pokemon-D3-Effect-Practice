...
本文件於 2026-09-19 隨 Roadmap 第一批實作更新。驗收範圍與執行結果見 [驗收紀錄](docs/verification/roadmap-batch-1.md)。
...
| `/pokemon` | 依英文名稱或編號搜尋、依任一屬性篩選、只顯示傳說；條件可透過網址分享及還原 |
| `/pokemon/[id]?form=...` | 指定型態的個別資料、能力條、防禦弱點／抗性／免疫分組與可展開的攻擊相剋 |
...
接續工作是圖片對應（#16）、卡片分段瀏覽（#19）與散佈圖詳細頁連結（#20）。目前圖片仍沿用舊有編號索引，Mega 圖片可能與型態不一致；完整列表仍一次載入。
